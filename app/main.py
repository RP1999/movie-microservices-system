...
app = FastAPI(
    ...
    servers=[
        {"url": f"http://localhost:{settings.port}", "description": "Direct Movie Service access"},
        {"url": settings.gateway_base_url, "description": "Example API Gateway access"},
    ],
    lifespan=lifespan,
)
...
async def service_overview() -> dict[str, str]:
    return {
        "message": "Movie Service backend is running.",
        "docs": "/docs",
        "redoc": "/redoc",
        "health": "/health",
        "api": "/api/movies",
        "gateway_docs": settings.gateway_base_url.replace("/api/movies", "/docs"),
    }
